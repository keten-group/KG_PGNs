...
def generate_chain_matrix(atom_data, bond_data, N):
...
def write_lammps_data_file(R, N, atom_num, bond_num, angle_num, atom_types, bond_types, angle_types, dimensions, masses, unwrapped_atom_data, bond_data, angle_data):
    # Re-Writing data file
    filename_new = f"R{R}_rho{gd}_N{N}_new.data"
...
def process_simulation_data(R, gd, N, blength, filename):
    """
    Process simulation data and save results.
    ...
    # Generate chain matrix
    chain_matrix = generate_chain_matrix(atom_data, bond_data, N)

    # Unwrap coordinates
    unwrapped_atom_data = unwrap_coordinates(atom_data, chain_matrix, dimensions, blength)

    # Write LAMMPS data file
    write_lammps_data_file(R, N, atom_num, bond_num, angle_num, atom_types, bond_types, angle_types, dimensions, masses, unwrapped_atom_data, bond_data, angle_data)

    # Save chain matrix
    np.savetxt(f'R{R}_rho{gd}_N{N}_chain_id.txt', chain_matrix, delimiter=',', fmt='%s')
...
filename = f"/projects/p31790/arman/KG_PGN/PGN_R{R}_rho{gd}_N{N}/KG/KG_R{R}_N{N}/equ_bond_swap/simulations/quench3/restart/restart.data"

process_simulation_data(R, gd, N, blength, filename)
